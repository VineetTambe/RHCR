...
{
  std::vector<grid_pathfinding::Path> tmp(num_agents, grid_pathfinding::Path(0));
  paths = tmp;
  makespan = 0;
}

grid_pathfinding::Path Paths::get(int i) const
{
...
grid_pathfinding::Node* Paths::get(int i, int t) const
{
...
grid_pathfinding::Node* Paths::last(int i) const { return get(i, makespan); }
...
void Paths::insert(int i, const grid_pathfinding::Path& path)
{
  const int paths_size = paths.size();
  if (!(0 <= i && i < paths_size)) halt("invalid index");
...
void Paths::operator+=(const Paths& other)
{
  ...
  if (makespan == 0) {  // empty
    ...
    std::vector<grid_pathfinding::Path> new_paths(paths_size);
    for (int i = 0; i < paths_size; ++i) {
      if (paths[i].empty() || *(paths[i].end() - 1) != other.paths[i][0]) {
        halt("invalid operation");
      }
      grid_pathfinding::Path tmp;
      // former
...
int Paths::countConflict(int id, const grid_pathfinding::Path& path) const
{
  int cnt = 0;
  int makespan = getMakespan();
  int num_agents = size();
