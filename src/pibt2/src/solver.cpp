...
// using namespace grid_pathfinding;
...
{
  grid_pathfinding::Grid* grid = reinterpret_cast<grid_pathfinding::Grid*>(P->getG());
...
void MAPF_Solver::makeLogSolution(std::ofstream& log)
{
  ...
  log << "starts=";
  for (int i = 0; i < P->getNum(); ++i) {
    grid_pathfinding::Node* v = P->getStart(i);
    log << "(" << v->pos.x << "," << v->pos.y << "),";
  }
  log << "\ngoals=";
  for (int i = 0; i < P->getNum(); ++i) {
    grid_pathfinding::Node* v = P->getGoal(i);
    log << "(" << v->pos.x << "," << v->pos.y << "),";
  }
  log << "\n";
  log << "solution=\n";
...
// distance
// -------------------------------
int MAPF_Solver::pathDist(const int i, grid_pathfinding::Node* const s) const
{
  if (distance_table_p != nullptr) {
...
void MAPF_Solver::createDistanceTable()
{
  for (int i = 0; i < P->getNum(); ++i) {
    ...
    std::queue<grid_pathfinding::Node*> OPEN;
    grid_pathfinding::Node* n = P->getGoal(i);
    OPEN.push(n);
...
// -------------------------------
MinimumSolver::AstarNode::AstarNode(grid_pathfinding::Node* _v, int _g, int _f, AstarNode* _p)
    : v(_v), g(_g), f(_f), p(_p), name(getName(_v, _g))
{
}

std::string MinimumSolver::AstarNode::getName(grid_pathfinding::Node* _v, int _g)
{
  return std::to_string(_v->id) + "-" + std::to_string(_g);
}

grid_pathfinding::Path MinimumSolver::getPathBySpaceTimeAstar(
    grid_pathfinding::Node* const s, grid_pathfinding::Node* const g, AstarHeuristics& fValue,
    ...
    CheckInvalidAstarNode& checkInvalidAstarNode, const int time_limit)
{
  ...
  auto createNewNode = [&GC](grid_pathfinding::Node* v, int g, int f, AstarNode* p) {
  ...
  while (!OPEN.empty()) {
    ...
    // expand
    grid_pathfinding::Nodes C = n->v->neighbor;
    C.push_back(n->v);
    for (auto u : C) {
      int g_cost = n->g + 1;
...
grid_pathfinding::Path MAPF_Solver::getPrioritizedPath(
    const int id, const Paths& paths, const int time_limit,
    const int upper_bound,
    const std::vector<std::tuple<grid_pathfinding::Node*, int>>& constraints,
    CompareAstarNode& compare, const bool manage_path_table)
{
  grid_pathfinding::Node* const s = P->getStart(id);
  grid_pathfinding::Node* const g = P->getGoal(id);
...
void MAPF_Solver::updatePathTableWithoutClear(const int id, const grid_pathfinding::Path& p,
                                              const Paths& paths)
{
  if (p.empty()) return;
...
int MAPD_Solver::pathDist(grid_pathfinding::Node* const s, grid_pathfinding::Node* const g) const
{
