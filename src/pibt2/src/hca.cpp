...
void HCA::run()
{
  ...
  for (int j = 0; j < P->getNum(); ++j) {
    ...
    // get path
    grid_pathfinding::Nodes path = getPrioritizedPath(i, paths);
...
// failed -> return {}
grid_pathfinding::Path HCA::getPrioritizedPath(int id, const Paths& paths)
{
  grid_pathfinding::Node* s = P->getStart(id);
  grid_pathfinding::Node* g = P->getGoal(id);

  grid_pathfinding::Nodes config_s = P->getConfigStart();
  grid_pathfinding::Nodes config_g = P->getConfigGoal();
