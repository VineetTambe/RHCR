...
grid_pathfinding::Node* Plan::get(const int t, const int i) const
{
...
grid_pathfinding::Path Plan::getPath(const int i) const
{
...
grid_pathfinding::Node* Plan::last(const int i) const
{
  if (empty()) halt("invalid operation");
...
int Plan::getPathCost(const int i) const
{
  ...
  const grid_pathfinding::Node* g = get(makespan, i);
  int c = makespan;
...
bool Plan::validate(const Config& starts) const
{
  ...
  for (int t = 1; t <= getMakespan(); ++t) {
    ...
    for (int i = 0; i < num_agents; ++i) {
      grid_pathfinding::Node* v_i_t = get(t, i);
      grid_pathfinding::Node* v_i_t_1 = get(t - 1, i);
      grid_pathfinding::Nodes cands = v_i_t_1->neighbor;
      ...
      for (int j = i + 1; j < num_agents; ++j) {
        grid_pathfinding::Node* v_j_t = get(t, j);
        grid_pathfinding::Node* v_j_t_1 = get(t - 1, j);
        if (v_i_t == v_j_t) {
...
int Plan::getMaxConstraintTime(const int id, grid_pathfinding::Node* s, grid_pathfinding::Node* g, grid_pathfinding::Graph* G) const
{
  const int makespan = getMakespan();
