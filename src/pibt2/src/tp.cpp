...
void TP::run()
{
  std::vector<grid_pathfinding::Path> TOKEN(P->getNum());
  Agents A;
  ...
  for (int i = 0; i < P->getNum(); ++i) {
    grid_pathfinding::Node* s = P->getStart(i);
    Agent* a = new Agent{
        i,        // id
        s,        // current location
        nullptr,  // task
  ...
  {
    grid_pathfinding::Nodes targets;
    Tasks tasks;
    for (auto a : A) {
...
void TP::updatePath1(int i, Task* task, std::vector<grid_pathfinding::Path>& TOKEN)
{
...
void TP::updatePath2(int i, std::vector<grid_pathfinding::Path>& TOKEN, Tasks& unassigned_tasks)
{
  info("   ", "updatePath2, agent-", i);

  grid_pathfinding::Node* target;
  grid_pathfinding::Node* loc = TOKEN[i][P->getCurrentTimestep()];
...
void TP::updatePath(int i, grid_pathfinding::Node* g, std::vector<grid_pathfinding::Path>& TOKEN)
{
