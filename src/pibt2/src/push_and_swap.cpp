...
bool PushAndSwap::push(Plan& plan, const int id, grid_pathfinding::Nodes& U,
                       std::vector<int>& occupied_now)
{
  if (plan.last(id) == P->getGoal(id)) return true;

  // create shortest path
  grid_pathfinding::Path p_star = getShortestPath(id, plan.last(id), occupied_now);
  p_star.erase(p_star.begin());
  if (p_star.empty()) return true;  // for safety

  grid_pathfinding::Node* v = p_star[0];
...
bool PushAndSwap::swap(Plan& plan, const int r, grid_pathfinding::Nodes& U,
                       std::vector<int>& occupied_now)
{
  ...
  bool succcess = false;
  grid_pathfinding::Nodes swap_verticies = nodes_with_many_neighbors;
  // sort, make swap operation easy
  {
    grid_pathfinding::Node* v = p_star[0];
    std::sort(swap_verticies.begin(), swap_verticies.end(),
              [v](grid_pathfinding::Node* a, grid_pathfinding::Node* b) {
  ...
  while (!swap_verticies.empty() && !succcess) {
    grid_pathfinding::Node* v = swap_verticies[0];
...
bool PushAndSwap::resolve(Plan& plan, const int r, const int s, grid_pathfinding::Nodes& U,
                          std::vector<int>& occupied_now)
{
  if (emergency_stop) return false;
  ...
  grid_pathfinding::Node* ideal_loc_s = plan.last(r);
...
bool PushAndSwap::multiPush(Plan& plan, const int r, const int s, const grid_pathfinding::Path& p,
                            std::vector<int>& occupied_now)
{
  if (emergency_stop) return false;
...
bool PushAndSwap::clear(Plan& plan, grid_pathfinding::Node* v, const int r, const int s,
                        std::vector<int>& occupied_now)
{
  if (emergency_stop) return false;
  ...
  // trivial case
  grid_pathfinding::Nodes unoccupied_nodes = getUnoccupiedNodes();
...
{
  // identify empty loc
  grid_pathfinding::Node* empty1 = nullptr;
  grid_pathfinding::Node* empty2 = nullptr;
  grid_pathfinding::Node* v = plan.last(r);
  grid_pathfinding::Node* last_loc_s = plan.last(s);
  for (auto u : v->neighbor) {
...
void PushAndSwap::updatePlan(const int id, grid_pathfinding::Node* next_node, Plan& plan,
                             std::vector<int>& occupied_now)
{
  // error check
...
bool PushAndSwap::pushTowardEmptyNode(grid_pathfinding::Node* v_current, Plan& plan,
                                      std::vector<int>& occupied_now,
                                      const grid_pathfinding::Nodes& obs)
{
  grid_pathfinding::Node* v_empty = getNearestEmptyNode(v_current, occupied_now, obs);
...
grid_pathfinding::Path PushAndSwap::getShortestPath(const int id, grid_pathfinding::Node* s,
                                  std::vector<int>& occupied_now)
{
  grid_pathfinding::Nodes p = {s};
  grid_pathfinding::Node* g = P->getGoal(id);
  if (s == g) return {};
  while (*(p.end() - 1) != g) {
    grid_pathfinding::Node* v = *(p.end() - 1);
    p.push_back(*std::min_element(v->neighbor.begin(), v->neighbor.end(),
                                  [&](grid_pathfinding::Node* a, grid_pathfinding::Node* b) {
                                    // path distance
...
grid_pathfinding::Node* PushAndSwap::getNearestEmptyNode(grid_pathfinding::Node* v, std::vector<int>& occupied_now,
                                       const grid_pathfinding::Nodes& obs)
{
  const int id = occupied_now[v->id];
  grid_pathfinding::Node* v_empty = nullptr;
  std::queue<int> OPEN;
  ...
  while (!OPEN.empty()) {
    ...
    if (CLOSE[i]) continue;
    CLOSE[i] = true;
    grid_pathfinding::Node* u = G->getNode(i);
    if (occupied_now[i] == NIL) {
      v_empty = u;
      break;
    }
    grid_pathfinding::Nodes C;
    for (auto w : u->neighbor) {
      if (CLOSE[w->id]) continue;
      C.push_back(w);
    }
    std::sort(C.begin(), C.end(), [&](grid_pathfinding::Node* a, grid_pathfinding::Node* b) {
...
Plan PushAndSwap::compress(const Plan& plan)
{
  ...
  while (!sameConfig(new_plan.last(), P->getConfigGoal())) {
    ...
    for (int i = 0; i < P->getNum(); ++i) {
      ...
      grid_pathfinding::Node* v_current = plan.get(t, i);
      ...
      grid_pathfinding::Node* v_next = plan.get(t + 1, i);
