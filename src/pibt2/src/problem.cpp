...
Problem::Problem(std::string _instance, grid_pathfinding::Graph* _G,
                 std::mt19937* _MT, Config _config_s, Config _config_g,
...
grid_pathfinding::Node* Problem::getStart(int i) const {
  if (!(0 <= i && i < (int)config_s.size())) halt("invalid index");
  return config_s[i];
}

grid_pathfinding::Node* Problem::getGoal(int i) const {
...
MAPF_Instance::MAPF_Instance(grid_pathfinding::Grid* G_)
    : Problem(), instance_initialized(true) {
  G = G_;
...
std::pair<int, int> MAPF_Instance::get_row_col(const int& id) {
  int row = (int)(id / G->getWidth());
  int col = id % G->getWidth();
  return {row, col};
}

void MAPF_Instance::setStartsGoals(const std::vector<int>& starts,
                                   const std::vector<int>& goals) {
  num_agents = starts.size();

  for (int i = 0; i < num_agents; ++i) {
    if (!G->existNode(starts[i])) {
      halt("start node with id (" + std::to_string(starts[i]) +
           ") does not exist, invalid scenario");
    }
    if (!G->existNode(goals[i])) {
      halt("goal node with id (" + std::to_string(goals[i]) +
           ") does not exist, invalid scenario");
    }

    grid_pathfinding::Node* s = G->getNode(starts[i]);
    grid_pathfinding::Node* g = G->getNode(goals[i]);
...
MAPF_Instance::MAPF_Instance(const std::string& _instance)
  ...
  while (getline(file, line)) {
    ...
    if (std::regex_match(line, results, r_map)) {
      G = new grid_pathfinding::Grid(results[1].str());
      continue;
    }
    // set agent num
    ...
    if (std::regex_match(line, results, r_sg) && read_scen &&
      ...
      grid_pathfinding::Node* s = G->getNode(x_s, y_s);
      grid_pathfinding::Node* g = G->getNode(x_g, y_g);
      config_s.push_back(s);
      config_g.push_back(g);
    }
  }
...
void MAPF_Instance::setRandomStartsGoals() {
  ...
  // get grid size
  grid_pathfinding::Grid* grid = reinterpret_cast<grid_pathfinding::Grid*>(G);
