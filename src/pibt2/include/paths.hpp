...
struct Paths {
private:
  std::vector<grid_pathfinding::Path> paths;  // main
  int makespan;
...
public:
  ...
  // agent -> path
  grid_pathfinding::Path get(int i) const;

  // agent, timestep -> location
  grid_pathfinding::Node* get(int i, int t) const;

  // return last node
  grid_pathfinding::Node* last(int i) const;
  ...
  // insert new path
  void insert(int i, const grid_pathfinding::Path& path);
  ...
  int countConflict(int id, const grid_pathfinding::Path& path) const;
