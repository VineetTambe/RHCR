...
protected:
  ...
  grid_pathfinding::Graph* const G;           // graph
...
protected:
  ...
  // space-time A*
  struct AstarNode {
    grid_pathfinding::Node* v;           // location
    int g;             // time
    int f;             // f-value
    AstarNode* p;      // parent
    std::string name;  // name
    AstarNode(grid_pathfinding::Node* _v, int _g, int _f, AstarNode* _p);
    static std::string getName(grid_pathfinding::Node* _v, int _g);
  };
   ...
   */
  static grid_pathfinding::Path getPathBySpaceTimeAstar(
      grid_pathfinding::Node* const s,                 // start
      grid_pathfinding::Node* const g,                 // goal
...
public:
  int pathDist(grid_pathfinding::Node* const s, grid_pathfinding::Node* const g) const { return G->pathDist(s, g); }
  int pathDist(const int i,
               grid_pathfinding::Node* const s) const;  // get path distance between s -> g_i
...
public:
  ...
  grid_pathfinding::Path getPath(grid_pathfinding::Node* const s, grid_pathfinding::Node* const g, bool cache = false) const
  {
    return G->getPath(s, g, cache);
  }
  // prioritized planning
  grid_pathfinding::Path getPrioritizedPath(
...
protected:
  ...
  void updatePathTableWithoutClear(const int id, const grid_pathfinding::Path& p,
                                   const Paths& paths);
...
protected:
  ...
  std::vector<grid_pathfinding::Nodes> hist_targets;  // time, agent -> current target
...
protected:
  ...
  DistanceTable distance_table;                         // distance table
  int pathDist(grid_pathfinding::Node* const s, grid_pathfinding::Node* const g) const;
