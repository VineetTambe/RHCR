...
using Config = std::vector<grid_pathfinding::Node*>;  // < loc_0[t], loc_1[t], ... >
...
[[maybe_unused]] static int getPathCost(const grid_pathfinding::Path& path)
{
...
protected:
  ...
  grid_pathfinding::Graph* G;              // graph
  std::mt19937* MT;      // seed
...
public:
  ...
  Problem(std::string _instance, grid_pathfinding::Graph* _G, std::mt19937* _MT, Config _config_s,
          Config _config_g, int _num_agents, int _max_timestep,
          int _max_comp_time);
  ~Problem(){};

  grid_pathfinding::Graph* getG() { return G; }
  int getNum() { return num_agents; }
  std::mt19937* getMT() { return MT; }
  grid_pathfinding::Node* getStart(int i) const;  // return start of a_i
  grid_pathfinding::Node* getGoal(int i) const;   // return  goal of a_i
...
private:
  ...
  std::pair<int,int> get_row_col(const int &id);

public:

  // MAPF_Instance(const BasicGraph& G);
  MAPF_Instance(grid_pathfinding::Grid* G_);
  ...
  void setStartsGoals(const std::vector<int> &starts, const std::vector<int> &goals);
};
...
private:
  ...
  grid_pathfinding::Nodes LOCS_PICKUP;             // candidates of pickup locations
  grid_pathfinding::Nodes LOCS_DELIVERY;           // candidates of delivery locations
  grid_pathfinding::Nodes LOCS_NONTASK_ENDPOINTS;  // endpoints, not necessary for PIBT
  grid_pathfinding::Nodes LOCS_ENDPOINTS;          // pickup, delivery, nontasks
...
public:
  ...
  grid_pathfinding::Nodes getEndpoints() { return LOCS_ENDPOINTS; }
};
